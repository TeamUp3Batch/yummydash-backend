...
const getRestaurantsByRating = async (req, res) => {
    try {
        const restaurants = await Restaurant.find().sort({ ratings: -1 }) // Sort by 'ratings' in descending order
...
const getRestaurantMenusByPrice = async (req, res) => {
    try {
        const menus = await Restaurant.find().sort({ 'menu.price': 1 })
...
const getMenusBelowDeliveryMinTime = async (req, res) => {
    try {
        ...
        const menus = await Restaurant.aggregate([
            ...
            {
                $match: {
                    'estimatedDeliveryTime.minEstimatedTime': {
                        $lte: minEstimatedTime,
                    },
                },
            },
            {
        ...
        if (!menus || menus.length === 0) {
            return res.status(404).json({
                message:
                    ('No menus found with delivery time less',
                    minEstimatedTime),
            })
        }
...
const getMenusBelowDeliveryMedTime = async (req, res) => {
    try {
        ...
        const menus = await Restaurant.aggregate([
            ...
            {
                $match: {
                    'estimatedDeliveryTime.medEstimatedTime': {
                        $lte: medEstimatedTime,
                    },
                },
            },
            {
...
const getMenusBelowDeliveryMaxTime = async (req, res) => {
    try {
        ...
        const menus = await Restaurant.aggregate([
            ...
            {
                $match: {
                    'estimatedDeliveryTime.maxEstimatedTime': {
                        $lte: maxEstimatedTime,
                    },
                },
            },
            {
...
const updateRestaurantDetails = async (req, res) => {
    try {
        ...
        if (!restaurantId) {
            return res
                .status(400)
                .json({ message: 'Missing required restaurantId' })
        }
